    ...
    return (
        <div className='lg:w-full lg:h-screen lg:bg-sidebar lg:col-span-2 lg:text-sm hidden lg:block'>
            ...
            <div className='flex-col items-center justify-evenly p-5'>
                ...
                {channelData.value.map((item: any, index: any) => ( // Use 'data.map' to render dynamic content
                    <div className='flex mb-2  shadow-sm hover:bg-slate-100 rounded-md cursor-pointer' key={index}>
                        <h3>#</h3>
                       
                            <Link href={`/thread/${item.id}`}>
                                <p onClick={() => dispatch(currentChannel(item.channelName))} className='ml-2'>
                                    {item.channelName}
                                </p>
                            </Link>
